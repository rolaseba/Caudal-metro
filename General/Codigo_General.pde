...
{
 var1 = 0;
while(var1!=11) //mientras no presione *
{
  //**Me fijo si quiero salir de la funcion, chequeando la tecla *
  digitalWrite(Columna3b, LOW);
  digitalWrite(Columna3c, HIGH);  
  F4c = digitalRead(Fila4c);
  if(F4c == HIGH)
...
{  
   ...
   while(var1!=11) //mientras no presione *
   {
     ...
     { 
      delay(50); 
      if(F4c==digitalRead(Fila4c)) //doble lectura de la tecla por el rebote
       {
        var1=11;//tecla *
       }
      }//cierro if 1
